...
#[test]
fn reth_http_plan_captures_chain_and_all_execution_head_states() {
    let config = RethSourceConfig::new("reth-eu-1", "http://127.0.0.1:8545", 1).expect("config");

    let plan = config.http_poll_plan().expect("HTTP plan");

    assert_eq!(plan.len(), 4);
    assert_rpc(&plan[0], "eth_chainId", &[]);
    assert_rpc(
        &plan[1],
...
fn websocket_endpoint_is_not_silently_reinterpreted_as_http() {
    let config = RethSourceConfig::new("reth-eu-1", "ws://127.0.0.1:8546", 1).expect("config");
